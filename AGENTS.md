...
# POIZON API (절대 NEXT_PUBLIC_ 접두사 사용 금지!)
POIZON_APP_KEY=your_app_key_here
POIZON_APP_SECRET=your_app_secret_here

# Naver Search API (절대 NEXT_PUBLIC_ 접두사 사용 금지!)
NAVER_CLIENT_ID=your_client_id_here
NAVER_CLIENT_SECRET=your_client_secret_here

# Exchange Rate API (선택 사항)
EXCHANGE_RATE_API_KEY=your_api_key_here
```

**중요**: POIZON과 Naver API 키는 **절대 클라이언트에 노출되어서는 안 됩니다**. `NEXT_PUBLIC_` 접두사를 사용하지 마세요. Server Actions와 API Routes에서만 접근하세요.
